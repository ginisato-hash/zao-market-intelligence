...
import {
  ...
  renderRakutenKnownIframeReport,
  type RakutenKnownIframeProbeRow
} from "../services/rakutenKnownIframeProbe";

const REPORT_DIR = ".data/reports/source-discovery";
const DEBUG_ROOT = ".data/debug/rakuten-known-iframe-probe";
...
async function probeOne(
  ...
  const context = await browser.newContext({
    userAgent:
      "Mozilla/5.0 (compatible; zao-market-intelligence-rakuten-known-iframe-probe/0.1; low-volume feasibility)"
  });
...
export async function runRakutenKnownIframeProbe(options: { timeoutMs?: number } = {}): Promise<{
  rows: RakutenKnownIframeProbeRow[];
  decision: ReturnType<typeof decideRakutenKnownIframeFeasibility>;
  csvPath: string;
  reportPath: string;
  ...
  let executionNote = "completed rakuten known iframe probe";
  try {
  ...
  const rows: RakutenKnownIframeProbeRow[] = [];
  for (const stayDate of STAY_DATES) {
    ...
    const classification = classifyRakutenKnownIframeProbe({ reachable: obs.reachable, evidence });
    const row: RakutenKnownIframeProbeRow = {
      canonicalPropertyName: PROPERTY.canonicalPropertyName,
  ...
  const decision = decideRakutenKnownIframeFeasibility(rows.map((row) => row.classification));
  const csvPath = resolve(REPORT_DIR, `rakuten_known_iframe_probe_${ts}.csv`);
  const reportPath = resolve(REPORT_DIR, `rakuten_known_iframe_probe_${ts}.md`);
  writeFileSync(csvPath, renderRakutenKnownIframeCsv(rows), "utf8");
  writeFileSync(
    reportPath,
    renderRakutenKnownIframeReport({
...
function riskNoteFor(classification: string): string {
  switch (classification) {
    case "known_iframe_date_scoped_total_found":
      return "Date-scoped 2-adult/1-room/1-night total evidence was detected in the known iframe text; review selectors before DB collection.";
    case "known_iframe_date_scoped_per_person_found":
      return "Known iframe exposed date-scoped per-person evidence, but not a safe total.";
    case "known_iframe_no_plan_or_sold_out":
      return "Known iframe reached an explicit no-plan/sold-out state.";
    case "known_iframe_date_scope_unverified":
      return "Known iframe opened but target date could not be confirmed in rendered text.";
    case "known_iframe_basis_unverified":
      return "Known iframe opened but adult/room/night/total basis remains unclear.";
    default:
      return "Known iframe URL failed or did not return usable Rakuten content.";
  }
}

async function main(): Promise<void> {
  const result = await runRakutenKnownIframeProbe();
...
function countClassifications(rows: RakutenKnownIframeProbeRow[]): Record<string, number> {
...
if (process.argv[1]?.endsWith("probeRakutenKnownIframeUrl.ts")) {
  void main();
}
