// Phase JALAN-AUTO03X - bounded Jalan collection probe.
//
// Uses Playwright only for the 25 fixed AUTO02X Jalan property/date targets.
// Writes source-discovery report/debug artifacts and preview rows only. No
// history append, DB write/sync, AI context refresh, pricing CSV, or PMS output.
...
import {
  ...
  renderPreviewRowsCsv,
  renderReport,
  type JalanExtractionCandidate,
  type JalanNormalizedPreviewRow,
  type JalanPageResult,
  type JalanProbeOutputShape,
  type JalanProbeTarget,
  type ProbeAvailabilityStatus
} from "../services/jalanBoundedCollectionProbe";
...
const DEBUG_ROOT = ".data/debug/jalan-bounded-collection-probe";
...
async function collectTarget(input: {
  ...
  reportPath: string;
  csvPath: string;
}): Promise<{ pageResult: JalanPageResult; row: JalanNormalizedPreviewRow }> {
  let attempt = 0;
...
async function collectTargetOnce(input: {
  ...
  const page = await input.browser.newPage({
    userAgent: "Mozilla/5.0 (compatible; zao-market-intelligence-auto03x/0.1; bounded manual verification)"
  });
  ...
  const targetJsonPath = resolve(input.debugPath, "target-results", `${name}.json`);
  ...
  try {
    ...
    const pageResult: JalanPageResult = {
...
async function attemptBoundedPlanClick(page: Page): Promise<boolean> {
...
function extractCandidateFromState(input: {
  ...
  const base = {
    ...
    raw_text_excerpt: input.bodyText.slice(0, 1200),
  ...
  if (selected !== undefined && selected.priceValue !== undefined) {
    return {
      ...
      coupon_or_discount_evidence: hasCouponOrSuspiciousEvidence(selected.blockText),
  ...
  if (evidence.priceFound && evidence.priceBasis === "total_tax_included" && evidence.priceValue !== undefined) {
    return {
      ...base,
      coupon_or_discount_evidence: hasCouponOrSuspiciousEvidence(evidence.surroundingText ?? input.bodyText),
      room_or_plan_name: null,
      availability_status: "available",
...
function buildThrownFailure(input: {
  ...
  attemptCount: number;
  retryUsed: boolean;
}): { pageResult: JalanPageResult; row: JalanNormalizedPreviewRow } {
  const name = safeName(input.target);
  const targetJsonPath = resolve(input.debugPath, "target-results", `${name}.json`);
  const errorMessage = input.error instanceof Error ? input.error.message : "unknown_error";
  const candidate: JalanExtractionCandidate = {
...
function hasCouponOrSuspiciousEvidence(text: string): boolean {
  return /クーポン|ポイント|会員限定|割引|セール|タイムセール|じゃらん限定ポイント/u.test(text);
}
...
async function run(): Promise<void> {
  const ts = timestamp();
  const runId = `jalan_bounded_collection_probe_${ts}`;
  ...
  mkdirSync(resolve(debugPath, "target-results"), { recursive: true });

  const reportPath = resolve(REPORT_DIR, `jalan_bounded_collection_probe_${ts}.md`);
  const jsonPath = resolve(REPORT_DIR, `jalan_bounded_collection_probe_${ts}.json`);
  const csvPath = resolve(REPORT_DIR, `jalan_bounded_collection_probe_${ts}.csv`);
  ...
  const pageResults: JalanPageResult[] = [];
  const rows: JalanNormalizedPreviewRow[] = [];
  try {
  ...
  const summaries = buildSummaries({ targets, pageResults, rows });
  const decision = decideJalanBoundedCollectionProbe({
  ...
  const output: JalanProbeOutputShape = {
    run_id: runId,
    ...
    next_phase: "JALAN-AUTO04X — Jalan history append proposal"
  };
  ...
  writeFileSync(csvPath, renderPreviewRowsCsv(rows), "utf8");
  writeFileSync(
    reportPath,
    renderReport({
      generatedAtJst,
