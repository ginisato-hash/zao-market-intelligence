...
const DEBUG_ROOT = ".data/debug/booking-official-tax-fee-total-hardening";
const USER_AGENT =
  "Mozilla/5.0 (compatible; zao-market-intelligence-booking-official-tax-fee-total-hardening/0.1; low-volume feasibility)";
...
async function runBookingOfficialTaxFeeTotalHardening(options: { timeoutMs?: number } = {}): Promise<{
  reportPath: string;
  csvPath: string;
  jsonPath: string;
  ...
  try {
    for (const target of TARGETS) {
      for (const checkin of CHECKINS) {
        ...
        const adderExtraction = extractOfficialTaxFeeAdder(row.officialTaxFeeTextRaw);
        ...
        await writeFile(
          join(artifactDir, "official_tax_fee_candidates.json"),
          JSON.stringify(
            {
              official_tax_fee_text_raw: row.officialTaxFeeTextRaw,
              ...
              official_tax_fee_adder_parts: adderExtraction.parts,
              final_all_in_total_visible: finalAllInTotalVisible,
  ...
  const decision = decideB04A(rows);
  const csvPath = resolve(REPORT_DIR, `booking_official_tax_fee_total_hardening_${ts}.csv`);
  const reportPath = resolve(REPORT_DIR, `booking_official_tax_fee_total_hardening_${ts}.md`);
  const jsonPath = resolve(REPORT_DIR, `booking_official_tax_fee_total_hardening_${ts}.json`);
  const summary = {
    decision,
    pageLoadCount,
    pricePolicyVersion: rows[0]?.pricePolicyVersion ?? "booking_official_visible_adder_v1",
    rows,
    ...
    adderStatusCounts: countBy(rows.map((row) => row.officialTaxFeeAdderExtractionStatus))
  };
  writeFileSync(csvPath, renderB04ACsv(rows), "utf8");
...
runBookingOfficialTaxFeeTotalHardening()
  .then((result) => {
    ...
    console.log(`adder_status_counts=${JSON.stringify(countBy(result.rows.map((row) => row.officialTaxFeeAdderExtractionStatus)))}`);
